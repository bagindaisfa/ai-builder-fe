...
from app.utils.exc import DocumentExtractorError, FileDownloadError, TextExtractionError, UnsupportedFileTypeError
from app.config import Config as config
# Configure logger
logger = setup_logger('DocumentExtractorNode')
class DocumentExtractorNode:
    ...
    def execute(self):
        try:
            if not self.context.get('files'):
                error_msg = "No files found in the request context"
            ...
            for file in self.context.get('files', []):
                if not file.get('path') or not os.path.exists(file['path']):
                    error_msg = f"File not found: {file.get('path')}"
                    logger.error(error_msg)
                    return {
                        'status': 'error',
                        'error': {
                ...
                try:
                    ...
                    files.append(FileProperties(
                        name=file_name,
                        path=file_path,
                        mime_type=file.get('mime_type', ''),
                        extension='.'+file.get('extension', ''),
                        file_byte=file_content
                    ))
                    logger.info(f"Prepared file for processing: {file_name}")
                except Exception as e:
                    error_msg = f"Error reading file {file.get('path')}: {str(e)}"
            ...
            for file in files:
                try:
                    extracted_text = _extract_text_from_file(file)
                    result.append({
                        'file_name': file.name,
                        'status': 'success'
                    })
            ...
            return {
                'status': 'success' if all(r['status'] == 'success' for r in result) else 'partial_success',
                'results': result,
                'text': result_text
            }
...
def _extract_text_from_doc(file_content: bytes) -> str:
    ...
    if not config.UNSTRUCTURED_API_URL:
        ...
        with tempfile.NamedTemporaryFile(suffix=".doc", delete=False) as temp_file:
            ...
            with open(temp_file.name, "rb") as file:
                elements = partition_via_api(
                    ...
                    api_key=config.UNSTRUCTURED_API_KEY,  # type: ignore
                )
...
class FileProperties:
    def __init__(self, name, path, mime_type, extension, file_byte):
        self.name = name
        self.path = path
...
def _extract_text_from_file(file: FileProperties):
    if file.extension:
        extracted_text = _extract_text_by_file_extension(file_content=file.file_byte, file_extension=file.extension)
    elif file.mime_type:
        extracted_text = _extract_text_by_mime_type(file_content=file.file_byte, mime_type=file.mime_type)
    else:
